...
function plotPoints(path) {
  ...
  path.forEach((instruction) => {
    ...
    let xVal = newCoordinates[0];
    let yVal = newCoordinates[1];

    if (direction === "R") {
      for (var i = 0; i < distance; i++) {
        xVal += 1;
        array.push(`${xVal},${newCoordinates[1]}`);
      }
      newCoordinates.splice(0, 1, newCoordinates[0] + distance)
    } else if (direction === "L") {
      for (var i = 0; i < distance; i++) {
        xVal -= 1;
        array.push(`${xVal},${newCoordinates[1]}`);
      ...
      for (var i = 0; i < distance; i++) {
        yVal += 1;
        array.push(`${newCoordinates[0]},${yVal}`);
      }
      newCoordinates.splice(1, 1, newCoordinates[1] + distance)
    } else if (direction === "D") {
      for (var i = 0; i < distance; i++) {
        yVal -= 1;
        array.push(`${newCoordinates[0]},${yVal}`);
      ...
      console.log("Something went wrong! Invalid opcode: " + direction);
    }
...
// lol, brute force takes time
function getIntersections(line1, line2) {
  return line1.filter(element => line2.includes(element));
}

function getDistances(intersections) {
  let distances = [];
  intersections.forEach((point) => {
    point = point.split(",");
    distances.push(Math.abs(point[0]) + Math.abs(point[1]))
  })
  return distances;
}

function getNearestCrossPoint(input) {
  const lines = input.split(" ");
  const wire1 = plotPoints(lines[0].split(","));
  const wire2 = plotPoints(lines[1].split(","));

  const crosspoints = getIntersections(wire1, wire2);
  return Math.min(...getDistances(crosspoints));
}

getNearestCrossPoint(input);
